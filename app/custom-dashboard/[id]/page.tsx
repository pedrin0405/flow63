    ...
    <div className="flex h-screen overflow-hidden bg-[#FDFDFD]">
      ...
      <div className="flex-1 flex flex-col overflow-y-auto custom-scrollbar">
        ...
        <header className="sticky top-0 z-30 w-full bg-white/80 backdrop-blur-xl border-b border-gray-100 px-8 py-5 flex items-center justify-between shadow-sm">
          <div className="flex items-center gap-3">
            <button className="lg:hidden p-2 bg-gray-50 rounded-xl" onClick={() => setSidebarOpen(true)}><Menu size={20} /></button>
            <button onClick={() => router.push('/custom-dashboard')} className="p-2 text-gray-300 hover:text-primary transition-all border border-gray-100 hover:border-blue-50 rounded-xl"><ChevronLeft size={24} /></button>
            <h2 className="text-2xl font-black text-foreground tracking-tight">{dashboard?.nome}</h2>
            <span className="text-primary font-black hidden sm:block">| {dashboard?.unidade || 'PROJETO CENTRAL63'}</span>
          </div>
        ...
        <main id="dashboard-export-area" className="flex-1 p-8 md:p-12 max-w-[1800px] mx-auto w-full space-y-10 bg-[#FDFDFD]">
          {loading ? (
            <div className="flex flex-col items-center justify-center py-60">
              ...
              <p className="font-black text-gray-300 uppercase tracking-[0.3em] mt-6 text-xs">Mapeando Dashboard...</p>
            </div>
          ) : (
