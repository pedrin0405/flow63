    ...
    <div className={cn("w-full font-sans overflow-x-hidden relative transition-colors duration-500", isPreview ? "min-h-full bg-transparent" : "min-h-screen")} style={{ backgroundColor: tema.bg_color, color: tema.text_color }}>
              ...
              <div className="w-full relative">
                <div className={cn("flex pt-2 pb-6 snap-x snap-mandatory gap-4", "overflow-x-auto lg:grid lg:grid-cols-2 lg:overflow-x-visible lg:pb-0 lg:pt-0", "[&::-webkit-scrollbar]:h-1.5 [&::-webkit-scrollbar-track]:bg-transparent [&::-webkit-scrollbar-thumb]:bg-gray-400/40 [&::-webkit-scrollbar-thumb]:rounded-full", "[scrollbar-width:thin] [scrollbar-color:rgba(156,163,175,0.4)_transparent]")}>
                  {data.featured_properties.items.map((imovel: any) => (
                    <motion.div key={imovel.id} whileHover={{ y: -4 }} onClick={() => handlePropertyClick(imovel)} className={cn("flex-none snap-start relative rounded-[2rem] overflow-hidden group cursor-pointer", isPreview ? "w-[240px] h-[280px]" : "w-[260px] sm:w-[280px] lg:w-full h-[320px]")} style={minimalCardStyle(tema)}>
      ...
      {showWhatsApp && !isPreview && (
        <motion.a 
          ...
          target={data.whatsapp?.number ? "_blank" : undefined}
          onClick={handleWhatsAppClick}
          initial={{ scale: 0, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} whileHover={{ scale: 1.1, rotate: 5 }} whileTap={{ scale: 0.9 }} className={cn("fixed rounded-full flex items-center justify-center z-[100] transition-all bottom-8 left-8 w-14 h-14 shadow-lg", !data.whatsapp?.number && "opacity-50 grayscale cursor-help")} style={{ backgroundColor: "#25D366", color: "#ffffff" }}
        >
